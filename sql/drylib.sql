...
*******************************************************************************
 ...
 with license fees. Transactions can be proven with the following ways:
    Bitcoin: https://blockchain.com , https://btc.com
    Ethereum: https://etherscan.io , https://ethplorer.io
    Monero: https://www.monero.how/tutorial-how-to-prove-payment
            https://web.getmonero.org/resources/user-guides/prove-payment.html
 
 You must retain control of your Bitcoin, Ethereum or Monero account for as
 long as you use this library. If you cannot prove ownership of your Bitcoin,
 Ethereum or Monero account at any time, you must consider license revoked
 and must pay license fees from another account.
 ...
 License costs 0.0008 BTC (or 0.04 ETH or 0.125 XMR) per day per human user per
 machine.
...
*******************************************************************************
' license into #drylib_license
declare @ns varchar(100) = 'dry'
if not exists(select 1 from sys.schemas where name=@ns) begin
   exec('create schema ' + @ns)
   declare @license_sp nvarchar(max) =
      (select 'create procedure ' + @ns + '.LICENSE as print ''*******'
            + license + ''''
         from #drylib_license)
   exec(@license_sp)
end
drop table #drylib_license
